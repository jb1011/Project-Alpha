...
export interface GasSeedDeps {
  ...
  /** Await the seed tx is mined before the caller depends on the new balance. */
 ...
 * from the platform wallet and await its confirmation before moving on, so a caller that depends
 * on the seeded balance right after this returns (e.g. sending a tx from the just-seeded address)
 * never races an unmined seed tx. Returns the seed tx hashes (empty when nothing needed topping up).
 * Chain-injected (getBalance/sendNative/confirm) so it unit-tests without a node.
 */
export async function ensureNativeGas(targets: Address[], d: GasSeedDeps): Promise<Hex[]> {
  ...
  for (const to of targets) {
    ...
    const hash = await d.sendNative(to, d.target - balance);
    await d.confirm(hash);
    hashes.push(hash);
  }
  return hashes;
}
