...
| Demo entity (D18) | `FormationE2E_1` | `publicId 9f8003f5-4c70-435a-9980-9a54625691b7`, Arc agent id `886257` |
...
- [ ] **Step 4: Who holds the demo entity's Arc guardian key and its tenant (D22, task 8).** Read `guardian` from the entity's `/metadata` JSON. Ask Alex which key that is, and whether the entity's owner tenant (the SIWE wallet that onboarded it; `owner_tenant_id` in the database, visible to Alex in the dashboard as the connected wallet) is a wallet Alex controls, because task 8 mints the demo API key as that tenant. Expected answer: both are keys Alex holds; the guardian key goes into 1Password as `Demo Guardian Key`. **Do not proceed to task 14 if** the guardian is a browser wallet with no exportable key: the fallback is to onboard a fresh demo entity through the API with a guardian key generated into 1Password (the `cli.int.test.ts` bootstrap flow shows the calls), and to re-run this step.
...
**Must already be on `main`:** PR #120 (formation payments) and PR #126 (the legal-body check), both merged 2026-09-10. Task 0 complete. Branch: `feat/hedera-rail`.
...
- [ ] **Step 4: Commit.** `git add package.json package-lock.json` → `feat(hedera): bump x402 to 2.25, add core/hono/hedera, drop x402-fetch (task 1)`.
...
**Must already be on `main`:** pull request 1 merged with `--merge`; `cfg.hedera`, the entity columns, `setHederaIdentity`, `HederaMirror`, and `/verify` all present. Branch: `feat/hedera-identity` from `origin/main`.
...
- [ ] **Step 1: Golden vector, generated once.** In a scratch directory outside the repo: `npm init -y && npm i @hashgraphonline/standards-sdk@0.1.186` and run a 10-line script calling the SDK's `createUaid`/`HCS14Client` (name from `src/hcs-14/sdk.ts`) with `{ registry: "novicorpus", name: "FormationE2E_1", version: "1", protocol: "mcp", nativeId: "eip155:5042002:<demo treasury lowercased>", skills: [] }` and `{ uid: "886257" }`. Record the full `uaid:aid:…` string and the SDK's `canonicalJson` in the test and in Records. Delete the scratch directory.
- [ ] **Step 2: Failing test:** canonical JSON equals the recorded string exactly (key order `skills,name,nativeId,protocol,registry,version`; `registry` and `protocol` lowercased; `nativeId` trimmed and lowercased by **our** `uaidInputsFor`, not by `canonicalizeAgentData`, mirroring the SDK); `deriveUaid` equals the recorded UAID; `parseUaidNativeId` returns the CAIP-10; a renamed entity yields a different id.
