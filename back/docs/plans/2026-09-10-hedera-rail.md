...
- [ ] **Step 0: USER TASK: the asks to Martin (D28, D29).** Post in Discord, with the four questions from the start-now note: (5) the review and merge policy for the window and (6) how the two VPS deploys of PR 1 and PR 2 happen by Sunday 2026-09-14, both as the option tables in the design's "Asks of Martin, with options" (link the design on the branch or on `main`). Ask Martin to answer with an option number per row; Alex's preferences are marked there (VPS access for Alex; a 24-hour review window then self-merge). No database is asked for. Expected: two option numbers, recorded in Records with the date. **If the deploy answer is silent by Friday 2026-09-12 evening,** the D28 fallback applies: local backend, both demo entities local, tunnel URL as the metadata base.
...
**Must already be on `main`:** pull requests 1 and 2 merged with `--merge` and deployed to the VPS (task 16); `HederaDemo_1` linked on prod (task 16 step 1) and both demo entities registered with UAIDs (task 10); `NOVI_ATTESTATION_KEY` and `Demo Guardian Key` in 1Password (task 0). Branch: `feat/hedera-attest` from `origin/main`.
