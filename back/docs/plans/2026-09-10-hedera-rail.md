...
- **Demo-only artifacts are labeled so nobody runs them by mistake.** Anything that exists only for the ETHOnline 2026 demo or only for a local run, and is not part of the production build, lives under `scripts/demo/` (backend) or carries `demo` in its command name (client `demo-buyer`), opens with the header block `// DEMO ONLY (ETHOnline 2026, Hedera lane). Not part of the production build. Never run against a production database.` followed by one sentence on what it is for, refuses to start unless `HEDERA_DEMO_LOCAL=1` is set, refuses when `NODE_ENV=production`, and prints `DEMO ONLY` as its first output line. The README's Hedera section lists them under a "Demo-only, do not run in production" heading.
...
| Scripts | backend | `scripts/hedera-register-identity.mts`, `scripts/guardian-pause.mts`; demo only: `scripts/demo/seed-public-entity.mts` (`--from-prod <publicId> --tenant <address>`, guarded by `HEDERA_DEMO_LOCAL=1`) |
...
| Pre-merge entity (D28) | seeded `FormationE2E_1` row, name `DEMO-LOCAL FormationE2E_1` | inserted into the local database by `scripts/demo/seed-public-entity.mts` (task 6b) from public data only (`/transparency`, identity registry, factory); `ownerTenantId` set to the `Hedera Demo Guardian Key` address so the local API key flow works; the only row task 8 touches; never on prod |
| 1Password items | | `Hedera Testnet Treasury` (guardian on Hedera), `Hedera Spike Agent Key` (agent), new: `Hedera Platform Operator`, `Novi Corpus Attestation Key`, `Hedera Demo Guardian Key` |
...
- [x] **Step 1: Branch state.** Done 2026-09-11: `hedera` cut from `main` `6d67f49`. Run `git -C ~/Desktop/Solidity_Project_Files/arc-Circle/Project-Alpha-monorepo status --short && git log --oneline -1 && git fetch origin && git rev-list --count HEAD..origin/hedera`. Expected: empty status, HEAD `6d37398` or later, `0` behind. **Do not proceed if** behind: `git merge --ff-only origin/hedera` first and re-verify anchors in step 4 of "How every task runs".
- [x] **Step 2: Baseline test count.** Done 2026-09-11 on `6d67f49`: 259 files passed, 5 skipped; 2199 tests passed, 5 skipped; 301 s. `cd back/backend && npx vitest run 2>&1 | tail -5`. Record the `Tests N passed` line in Records. Every later task compares against it.
- [x] **Step 3: Demo entity is public on chain with a verified human.** Done 2026-09-11: `funded`, `humanVerified: True`, `agentId 886257`, standing `active`. `curl -s https://www.novicorpus.com/backend/transparency | python3 -c 'import sys,json; [print(e) for e in json.load(sys.stdin)["entities"] if e.get("publicId")=="9f8003f5-4c70-435a-9980-9a54625691b7"]'`. Expected: one line with `'status': 'funded'`, `'humanVerified': True`, `'agentId': '886257'`. Then `curl -s https://www.novicorpus.com/backend/legal-bodies/<treasury>` once the treasury is known from `/metadata/9f8003f5-…`'s `legalBody` block; expected `"standing":"active"`. **Do not proceed if** standing is not `active`: fall back to `TestBootstrapMB_1` (`061441c6-cc58-466b-b44f-5e627d823e09`) and update D18 in the design.
- [ ] **Step 4: Onboard the paying entity `HederaDemo_1` on prod (D18, D22).** `FormationE2E_1` was onboarded by Martin (confirmed by Alex 2026-09-10), so its tenant and guardian keys are not Alex's; `HederaDemo_1` does not exist on prod as of 2026-09-10 (16 entities, none by that name). Prod's `/config` says `formationPaymentRequired: false` and the doola environment is `sandbox`, so this costs nothing (pre-cleared). Generate a guardian key into 1Password as `Hedera Demo Guardian Key` (`private_key_hex`), and import that key into a browser wallet and onboard a new entity through the dashboard signed in with it: the server sets the guardian to the signed-in wallet (`src/api/routes/onboard.ts`, `roles.guardian = tenantId`), so the guardian key is the sign-in key, not a separate field. Custody `circle` (the platform default), name `HederaDemo_1`, allowlist empty, fund amount typed as `10`. The onboarding takes a real WebAuthn passkey even for `circle` custody, and prod enforces the World guardian gate with no skip: verify with World App or redeem a waiver code minted by Martin on the box (`npm run cli -- waiver create -n "Alex - HederaDemo_1 guardian" -d 3`). The platform manager pays the funding; the guardian wallet needs no USDC. Expected: the entity reaches `funded` (not `bound`: `check_policy` reads `legalStatus` on the proxy and `paused` on the treasury, so both must exist), appears on `/transparency` with a `publicId`, and `curl -s https://www.novicorpus.com/backend/legal-bodies/<its treasury>` answers `"standing":"active"`. Record its `publicId`, treasury, guardian. This entity links the Hedera float account (task 8), owns the demo API key (task 8 locally, task 16 on prod), is registered on Hedera (task 10), carries the memo (task 12) and is paused in task 14; `FormationE2E_1` is the one `/verify` answers about. **Do not proceed to task 14 if** the guardian is a browser wallet with no exportable key: the fallback is to onboard a fresh demo entity through the API with a guardian key generated into 1Password (the `cli.int.test.ts` bootstrap flow shows the calls), and to re-run this step.
- [x] **Step 5: USER TASK: 1Password items.** Done 2026-09-11: all three exist; guardian address `0x6AB681DbFa81CA0D4acec965bDEd3A17292Ac142`, attestor address `0x269EF6d269361ceBE28D10A400E5290e7f23CdC6`, each recorded on its item as `address`. Alex creates, in vault **Novi Corpus**: `Hedera Platform Operator` with `account_id=0.0.10412694` and `private_key_hex` (the portal key, ECDSA), `Novi Corpus Attestation Key` with `private_key_hex` (generate: `node -e "console.log('0x'+require('crypto').randomBytes(32).toString('hex'))"` and paste, never print again), `Hedera Demo Guardian Key` with `private_key_hex` from step 4. Expected: `op item get "Hedera Platform Operator" --vault "Novi Corpus" --fields label=account_id` prints `0.0.10412694`.
- [x] **Step 6: LIVE: associate the spare account with USDC (D26).** Done 2026-09-11: `TOKENASSOCIATE SUCCESS`, tx `0.0.10412694@1789157387.239566682`; mirror node lists `0.0.429274`. In `back/hedera-client` after task 7 exists, or now from the spike folder: `op run --env-file=.env.tpl -- npx tsx src/01-associate.ts` with `TREASURY_ACCOUNT_ID`/`TREASURY_PRIVATE_KEY` pointed at the `Hedera Platform Operator` item in a copy of `.env.tpl`. Expected: `TOKENASSOCIATE SUCCESS`, and `curl -s https://testnet.mirrornode.hedera.com/api/v1/accounts/0.0.10412694/tokens` lists `0.0.429274`. Record the consensus timestamp.
...
- [x] **Step 8: Testnet reset check.** Done 2026-09-11: none scheduled through 2026-09-16; the testnet upgrades of 2026-09-08 and 09-09 were canceled. Open `https://status.hedera.com/` and look for a scheduled testnet reset between now and 2026-09-16 (the last reset was 2024-02-01; later scheduled ones were skipped). Expected: none scheduled. **If one is scheduled inside the window,** record the date and move every live step ahead of it.
- [x] **Step 9: Local pre-merge entity (D28), rewritten 2026-09-11.** A locally onboarded entity is impossible: `LegalManagerFactory.createEntity` is `onlyOwner` with `manager == owner`, and the owner is the production controller `0xb43CbdA374e3CD2a3d67827683F81462BaCF703b` on Arc testnet, whose key never leaves the box; the sandbox profile also carries no Circle configuration. Alex decided 2026-09-11: the pre-merge row is seeded from public data by the demo-only script in task 6b. Nothing to run in task 0.
...
### Task 6b: Demo-only seed script for the local pre-merge row

**Files:**
- Create: `back/backend/scripts/demo/seed-public-entity.mts`
- Test: `back/backend/test/scripts/seedPublicEntity.test.ts`

**Interfaces:**
- Consumes: `openDatabase`, `migrate` (`src/persistence/db.ts`), `SqliteEntityRepository.upsert` (`src/persistence/entityRepository.ts`), `EntityRecord` (`src/types.ts:36`), viem `createPublicClient` with the Arc testnet chain (`src/adapters/arc/clients.ts`), `iIdentityRegistryAbi` (`src/abis/generated.ts`).
- Produces: one `EntityRecord` row with `status: "funded"`, `name: "DEMO-LOCAL " + name`, `publicId`, `agentId`, `proxy`, `treasury`, `manager`, `guardian`, `ownerTenantId: --tenant`, `walletProvider: "circle"`, every key and party field null. Exported pure function `buildSeedRecord(input: { transparency: TransparencyEntity; proxy: Address; treasury: Address; manager: Address; guardian: Address; tenant: Address }): EntityRecord` so the mapping is testable without a network.

- [ ] **Step 1: Failing test.** `test/scripts/seedPublicEntity.test.ts`: (a) `buildSeedRecord` maps a fixture transparency entry (`publicId`, `name`, `agentId`) plus addresses to a record whose `status` is `"funded"`, `name` starts with `DEMO-LOCAL `, `ownerTenantId` equals the tenant, and `turnkeySubOrgId`, `circleWalletSetId`, `pocketAddress`, `specJson` are all null or undefined; (b) `assertDemoGuard({ HEDERA_DEMO_LOCAL: undefined, NODE_ENV: "test" })` throws `Error("DEMO ONLY: set HEDERA_DEMO_LOCAL=1 to run this script")`; (c) `assertDemoGuard({ HEDERA_DEMO_LOCAL: "1", NODE_ENV: "production" })` throws `Error("DEMO ONLY: refuses to run with NODE_ENV=production")`. Run: `npx vitest run test/scripts/seedPublicEntity.test.ts`. Expected: FAIL, module not found.
- [ ] **Step 2: Script.** File opens with the header block from the global constraint, then `export function assertDemoGuard(env: NodeJS.ProcessEnv): void`, `export function buildSeedRecord(...)`, and a `main()` that runs only when the file is the entry point (`import.meta.url === pathToFileURL(process.argv[1]).href`). `main()`: `assertDemoGuard(process.env)`; `console.log("DEMO ONLY")`; parse `--from-prod <publicId>` and `--tenant <address>` (both required; `getAddress` on the tenant); fetch `https://www.novicorpus.com/backend/transparency` and pick the entry by `publicId` (exit 1 with `entity not found on /transparency` otherwise); read the on-chain values with a public client on `https://rpc.testnet.arc.network`: `proxy` and `treasury` from the factory's entity lookup (`LegalManagerFactory` read function for the agent id; if the factory exposes none, read the `EntityCreated` event log filtered by `agentId`), `manager` and `guardian` from the proxy's role getters; open `DATA_DIR` (default `./data`) with `openDatabase` + `migrate`, `upsert(buildSeedRecord(...))`; print `seeded DEMO-LOCAL <name> publicId <publicId> treasury <treasury>`. No key, no tenant session, no party data is read or written.
- [ ] **Step 3: Tests green; three checks.** `npx vitest run test/scripts/seedPublicEntity.test.ts` PASS; `npm run lint`, `npm run typecheck`, `npm test`.
- [ ] **Step 4: README line.** Under the Hedera section's "Demo-only, do not run in production" heading: the script's name, the one sentence on purpose, and the guard variable. **Commit** → `feat(hedera): demo-only seed script for the local pre-merge row (HEDERA_DEMO_LOCAL guard)`.
...
- [x] **Step 1:** Done 2026-09-12: seeded `0x6AB681DbFa81CA0D4acec965bDEd3A17292Ac142:FormationE2E_1` (name `DEMO-LOCAL FormationE2E_1`) into `./data-sandbox/legalbody.db` from the prod `/transparency` row and the production factory `0x83D529E813Fe825b84250034A7A63f460A2ECA77`, recovered from the `EntityCreated` log at Arc block 58927506 (the sandbox example's Lisbon-era factory `0x91997dFc…` and the July broadcast's `0x0d185b70…` both answer the zero address for agent 886257). Boot deviation, recorded: `assertLegacyFactoryOwner` refuses a local boot unless the signing key owns the factory and has no bypass, so the backend ran `npm run api:sandbox` against an Anvil fork of Arc testnet (`ARC_TESTNET_RPC_URL=http://127.0.0.1:8545`) with the factory's owner slot patched to Anvil's public dev account 0 and `PLATFORM_PRIVATE_KEY` set to that account's public dev key; the `HEDERA_*` values were exported inline with `PORT=8787`, no `.env.hedera.tpl` (the block holds no secret). Only Arc reads (paused, legal status, allowlist) came from the fork of live state; every Hedera hop was live. Boot line `Wizard API listening on :8787`; `/legal-bodies/0x92ae7c6b…` → `"standing":"active"`; unpaid `GET /verify/9f8003f5-…` → 402 with `accepts: [{scheme exact, network hedera:testnet, amount 1000, asset 0.0.429274, payTo 0.0.10412694, maxTimeoutSeconds 300}]`. Seed the local database, then start the backend with the flag on (D28): `cd back/backend && HEDERA_DEMO_LOCAL=1 npx tsx scripts/demo/seed-public-entity.mts --from-prod 9f8003f5-4c70-435a-9980-9a54625691b7 --tenant 0x6AB681DbFa81CA0D4acec965bDEd3A17292Ac142` (expected first line `DEMO ONLY`, last line `seeded DEMO-LOCAL FormationE2E_1 publicId 9f8003f5-… treasury 0x92ae…`), then `cd back/backend && op run --env-file=.env.hedera.tpl -- npm run api` where `.env.hedera.tpl` is a copy of the deployment's template with the `HEDERA_*` server values from the naming table (`HEDERA_PAYTO_ACCOUNT_ID=0.0.10412694`). Expected boot line includes `hedera: { network: 'testnet'` and `attestationKey: undefined`, and `curl -s http://127.0.0.1:8787/legal-bodies/0x92ae7c6b6eB9470d7E01F8fEb352714bD80A7AAf` answers `"standing":"active"` (a real chain read of `FormationE2E_1`'s treasury).
- [x] **Step 2:** Done 2026-09-12: key id `80b3023d-536b-477b-a481-e1b356ee710a`, capability `spend`, tenant `0x6AB681Db…`, minted through `/auth/nonce` → SIWE → `/auth/verify` → `POST /api-keys` by an untracked demo-only helper that wrote the key straight into 1Password (`Novi Corpus Demo API Key`, field `local`, item created); the key was never printed. Create an API key for the seeded row's tenant (the `Hedera Demo Guardian Key` address, signed in to the local backend) with `spend` capability through the existing `/api-keys` flow (the `cli.int.test.ts` sequence), store it in 1Password as `Novi Corpus Demo API Key` (field `local`). Expected: `whoami` over MCP returns Alex's tenant. This key exists only in the local database; task 16 mints the prod one into the same item (field `prod`).
- [x] **Step 3:** Done 2026-09-12: `provision` reused the spike agent key and account `0.0.10450558` (agent public key `03df549c…`); the 1-of-2 key-list update succeeded (`0.0.<treasury>@1789178239.633441419`), the command printed `order that worked: key list on a hollow account`, memo skipped until task 12. Because the account pre-existed, the hollow-account order was not exercised on a fresh account this run. The float account holds 0 HBAR. `cd back/hedera-client && op run --env-file=.env.tpl -- npx tsx src/cli.ts provision`. Expected last lines: `account 0.0.<new> key ThresholdKey(1 of 2)` and the printed link call. Record the account id and the two transaction ids.
- [x] **Step 4:** Done 2026-09-12: `{"ok":true,"accountId":"0.0.10450558","guardianPublicKey":"03afcdc1…"}`; row `hedera_linked_at 1789178290`. Link: `op run … -- npx tsx src/cli.ts link` (a thin command that calls `linkHederaAccount`). Expected: `{"ok":true,"accountId":"0.0.<new>","guardianPublicKey":"03afcd…"}`.
- [x] **Step 5:** Done 2026-09-12, first try: `HTTP 200`, body `{"subject":{"publicId":"9f8003f5-4c70-435a-9980-9a54625691b7","name":"DEMO-LOCAL FormationE2E_1",…`, `settlement: OK https://hashscan.io/testnet/transaction/0.0.7162784@1789178320.131369376`, `report_payment -> settled`. Mirror node: `CRYPTOTRANSFER SUCCESS`, consensus `1789178327.610444922`, fee payer `0.0.7162784`, USDC `0.0.429274` moved `0.0.10450558 -1000`, `0.0.10412694 +1000`. Screen capture: Alex's. Pay: `op run … -- npx tsx src/cli.ts pay http://127.0.0.1:8787/verify/9f8003f5-4c70-435a-9980-9a54625691b7`. Expected: `HTTP 200`, body starting `{"subject":{"publicId":"9f8003f5-4c70-435a-9980-9a54625691b7`, `settlement: OK https://hashscan.io/testnet/transaction/0.0.7162784@…`, then `report_payment -> settled`. Open the HashScan link: `CRYPTOTRANSFER SUCCESS`, 0.001 USDC to `0.0.10412694`. Record the transaction id and a screen capture at once: this is the D27 line and the evidence if Blocky402 is down later (audit U3). **Do not proceed if** the report stays `pending` after three tries: read the mirror node by timestamp, fix `mirrorTxId`, and rerun. **If Blocky402 answers 5xx or times out on every try,** run the facilitator locally from `@x402/hedera`'s `x402Facilitator` with the spare account as fee payer (the start-now note's fallback), point `HEDERA_FACILITATOR_URL` at it, and record that the settlement was self-facilitated.
- [x] **Step 6:** Done 2026-09-12: `hedera:testnet|settled|0.0.7162784-1789178320-131369376|1000` (database `./data-sandbox/legalbody.db`). `sqlite3 ./data/legalbody.db "select network, status, batch_ref, amount from payments_ledger order by id desc limit 1"`. Expected: `hedera:testnet|settled|0.0.7162784-<seconds>-<nanos>|1000` (the `batch_ref` is the canonical dashed mirror-node id that `mirrorTxId` produces, not the `@` form the client prints; both name the same transaction).
- [x] **Step 7:** Done 2026-09-12 through the commit: README section, these records and the D29 wording (docs ride in the code PR) in one `docs(hedera)` commit; backend suite rerun before it. Push and the pull request wait for Alex's word. README section (no key material, no 1Password field values, item titles only; the repo is public, audit U9); three checks; `superpowers:requesting-code-review` on the branch; **commit** → `docs(hedera): README section for the rail, live run recorded (task 8)`. Then `git push -u origin feat/hedera-rail` and `gh pr create --title "feat(hedera): the rail — self-custody payments on Hedera testnet, /verify unsigned (PR 1 of 3)" --body-file <a body listing tasks 1–8, the HashScan links, D1/D2/D15 in one paragraph each, and the AI-attribution line>`, reviewer Martin. **Merge step (ends phase 1):** D29: Martin has 24 hours; after that, CI green and Alex's diff read, `gh pr merge --merge --delete-branch` (D20), then `git checkout -b feat/hedera-identity origin/hedera`.
...
- [x] **Step 1: Failing test** Done 2026-09-12 (commit 7e5891c): fakes for simulate → write → receipt; success gives `{ agentId: "12", txHash }`, reverted throws `register reverted (tx …, status …)`. for `registerOnHedera` with fake viem clients: `simulateContract` returns `{ result: 12n, request }`, `writeContract` returns a hash, `waitForTransactionReceipt` returns `status: "success"` → `{ agentId: "12", txHash }`; receipt `reverted` → throws `register reverted`.
- [x] **Step 2: `registry.ts`:** Done 2026-09-12: `src/hedera/registry.ts` with `registerOnHedera` and the `HEDERA_IDENTITY_REGISTRY` constant; the account is a viem local account from `HEDERA_OPERATOR_KEY`, so the write is signed locally (a typecheck failure caught a bare-address account that would have sent `eth_sendTransaction` to Hashio). `simulateContract({ address: registry, abi: iIdentityRegistryAbi, functionName: "register", args: [metadataURI], account })` → `writeContract(request)` → `waitForTransactionReceipt`. The agent id is the simulate result (the ABI carries no event to parse).
- [x] **Step 3: Script** Done 2026-09-12: script with `--entity`, `--all-entities` (refused without `--execute --yes`), `--record`, `--from-prod`. Ruling: `--from-prod` never calls `loadConfig()` nor opens a database (the local `.env` holds a placeholder platform key), takes `chainId` from `ARC_CHAIN_ID` (default 5042002). Review fix round (abadbea): `--from-prod --execute` also requires `--yes`, else it prints and refuses (exit 2) before any key is read; `--record` re-derives the UAID from the row and refuses on mismatch (so a shell `ARC_CHAIN_ID` that differs from the box's `cfg.chainId` reads as a refusal, by design). `dotenv` is loaded lazily in `main()` so the test suite never loads a stray `.env`. 22 tests. `scripts/hedera-register-identity.mts --entity <name|key> [--entity …] | --all-entities [--execute]` and `--record --entity <id> --agent-id <n> --tx <hash> --uaid <uaid>`, idiom `scripts/sweep-standing-float.mts:1-40`: `loadConfig()` for `dbPath` and `chainId`; script-only env `HEDERA_JSON_RPC_URL`, `HEDERA_OPERATOR_KEY` read from `process.env` and refused if missing; the registry address is `HEDERA_IDENTITY_REGISTRY` exported from `src/hedera/registry.ts` (audit C3); viem chain object `{ id: 296, name: "hedera-testnet", nativeCurrency: { name: "HBAR", symbol: "HBAR", decimals: 18 }, rpcUrls: { default: { http: [rpc] } } }`; for each named entity with `metadataURI` and no `hederaAgentId`: dry-run prints the metadata URI and the UAID; `--execute` registers, derives the UAID with `uaidInputsFor(entity, cfg.chainId)` and `uid = entity.agentId`, calls `setHederaIdentity`, prints `hashscan.io/testnet/transaction/<txHash>` and the exact `--record` line to run on the box. `--all-entities` is refused without `--execute --yes` and prints the count first (audit C17: 16 entities on prod, other tenants' included). `--record` writes the three values with `setHederaIdentity` and no chain call, for the box (D28). `--from-prod <publicId>` (repeatable) takes the entity from prod's public endpoints instead of the local database, which under D28 never holds the prod rows: `GET https://www.novicorpus.com/backend/metadata/<publicId>` for the metadata URI (the URL itself) and the `legalBody` block, `/transparency` for `name`, `treasury` and `agentId`; it registers with that URI, derives the UAID from those values, and prints the `--record` line. It refuses to run without `--execute` against anything but `www.novicorpus.com`.
- [x] **Step 4: LIVE.** Done 2026-09-12 for `FormationE2E_1` only (`HederaDemo_1` has no prod row yet, World waiver pending): operator `0x6419…E64F`, `registered agentId=113 tx=0xc5389a0a6f38fdecb6792c0b07442026f86e3d710168c6ae108ecf855c521eb7`, UAID equals the task 9 golden vector; mirror node `SUCCESS`, consensus `1789183092.886182179`, gas 200820; `ownerOf(113)` on the registry is the operator. The `--record` line for the box is in the SDD workspace `drafts/task16-record-lines.md` and in task 16's records. Alex ran the send after the permission classifier blocked it here. `op run --env-file=.env.register.tpl -- npx tsx scripts/hedera-register-identity.mts --from-prod 9f8003f5-4c70-435a-9980-9a54625691b7 --from-prod <HederaDemo_1 publicId> --execute` (the operator key's EVM alias equals the account's `evm_address`, pre-cleared, so the write spends `0.0.10412694`'s HBAR). Expected, twice: `registered agentId=<n> tx=0x… uaid=uaid:aid:…` and a `--record …` line. Record both, and hand the two `--record` lines to task 16. The first test in step 1 also covers `--from-prod` with a stubbed `fetch`: the UAID it derives for `FormationE2E_1`'s public values equals the task 9 golden vector.
- [x] **Step 5:** Done 2026-09-12: commits 7e5891c and abadbea; typecheck, lint and 22 tests green; full suite at 7e5891c 269 files / 2317 tests green. Three checks; **commit** → `feat(hedera): ERC-8004 registration on Hedera testnet, identity script, UAID stored (task 10)`.
...
- [x] **Step 1: Failing test:** Done 2026-09-12 (commit 4c4e13b): 13 unit tests plus 4 route tests; the reviewer signed a body independently from the brief's domain and types and `verifyAttestation` accepted it. sign with `privateKeyToAccount(key).signTypedData`, verify with viem `verifyTypedData` → true; flip `standing` → false; `verifyAttestation` on the body a test route call returned → true.
- [x] **Step 2:** Done 2026-09-12: `attestationMessage` is the one shared flattener for sign and verify; null mapping = the brief's four plus `formation null → ""` for both formation fields and empty treasury → zero address; `issuedAtUnix`/`expiresAtUnix` added unconditionally and signed as `uint256`, the ISO pair unsigned; `attestor` and `signature` only with `NOVI_ATTESTATION_KEY`; `hedera.attestor` in `/metadata`. Review finding fixed in 7369181: `formation.filed`/`einIssued`, `subject.name`, `registry`, the credential, hex case and extra JSON keys are NOT under the signature; the README's "Signed attestation" section tells verifiers to read the twelve fields and derive the booleans from `formationStatus`. Deferred: bind `attestationMessage`'s return type to `ATTESTATION_TYPES`. Implement; the route adds `attestor` and `signature` when the key is present, and `issuedAt`/`expiresAt` are serialised as unix seconds strings in the typed data and ISO strings in the JSON body (state both in the body: `issuedAt`, `issuedAtUnix`, `expiresAt`, `expiresAtUnix`). Null mapping into the typed data, applied identically by `signAttestation` and `verifyAttestation` and stated in the README (design Component 3, audit C12): `oaHash null` → `0x` + 64 zeros, `manifestVersion null` → `0`, `uaid null` → `""`, `agentId null` → `""`. Add one test: a body with all four null signs and verifies, and flipping `oaHash` from the zero sentinel to any other value fails verification.
- [x] **Step 3:** Done 2026-09-12: full suite at 4c4e13b 271 files / 2358 tests green, no flake; commits 4c4e13b, 7369181. Tests; three checks; **commit** → `feat(hedera): EIP-712-signed legal-standing attestation on /verify (task 13)`.
...
- [x] **Step 1:** Done 2026-09-12 (commits 04cc5c2, 9454cc3): pure `assertGuardianMatches`, `parseArgs`, `configModeFor` tested (12 tests). Ruling: `--treasury` mode runs without `loadConfig()` and without a database (the local `.env` holds a placeholder platform key; same ruling as task 10's `--from-prod`), clients built from `ARC_TESTNET_RPC_URL` (default `https://rpc.testnet.arc.network`) and `ARC_CHAIN_ID` (default 5042002); `--entity` mode keeps `loadConfig()`. `--entity` reuses `resolveEntity` from `hedera-register-identity.mts`. `.env.guardian.tpl` maps `DEMO_GUARDIAN_KEY` only. Script `pause|unpause --entity <name|key> | --treasury <address>`: `loadConfig()`, find the entity in the local database or take the treasury address as given (under D28 the demo entity lives on prod, so the demo uses `--treasury`), read `guardian()` from the treasury with `publicClientFor(cfg)` and `agentTreasuryAbi` (`src/abis/generated.ts:8`), refuse unless `privateKeyToAccount(process.env.DEMO_GUARDIAN_KEY).address` equals it (`guardian key does not match on-chain guardian`), then `walletClientForKey(cfg, key).writeContract({ address: treasury, abi: agentTreasuryAbi, functionName: mode })` and wait for the receipt; print `paused()` after.
- [ ] **Step 2: LIVE round trip:** Pending: needs `HederaDemo_1` on prod (its guardian is `Hedera Demo Guardian Key`; `FormationE2E_1`'s guardian is Martin's) and PR 1 deployed for the prod `check_policy` leg. Runs with task 16. `op run --env-file=.env.guardian.tpl -- npx tsx scripts/guardian-pause.mts pause --treasury <HederaDemo_1 treasury>` (the paying entity, whose guardian key is `Hedera Demo Guardian Key`; `FormationE2E_1`'s guardian is Martin's, and the buyer's refusal comes from `check_policy` on the payer, D18, audit B1) → `paused: true`; `check_policy` over prod MCP for `HederaDemo_1` (PR 1 is deployed by then, task 16 step 1) → `{ ok: false, reason: "paused" }`; `unpause` → `paused: false`. Record both hashes.
- [x] **Step 3:** Done 2026-09-12: 04cc5c2, then the review fix 9454cc3. **Commit** → `feat(hedera): guardian pause/unpause script for the demo (task 14)`.
...
- [x] **Step 1:** Done 2026-09-12: aea3bf9, then the review fix aed0974. Two corrections from the build: the attestation field is `controller.humanVerified` (not `worldId.humanVerified`), and leg 2 uses `guardian-pause.mts pause --treasury <address>` (task 14's env-only mode; under D28 the demo entities live on prod and the local database has no row). The client verifies offline through its own copy of the canonicalization (`src/attest.ts`, viem pinned exact to the backend's resolved 2.52.2); the copy is checked by execution, not by CI. `demoBuyer`'s wiring (resolve → sign → pay → print → exit code) has no test of its own because it needs a real key and a live MCP transport, so step 2 is its test and is not optional. 21 new tests, 40 in the client suite. The spec as written: `demo-buyer <uaid>`: `parseUaidNativeId` → `GET https://www.novicorpus.com/backend/legal-bodies/<address>` (the base comes from `NOVI_API_BASE`, prod for the demo, D28) → follow `links.metadata` → read `hedera.profileUrl` and `hedera.verifyUrl` → `payFetchFor(...)` on `verifyUrl` → print `standing`, `humanVerified`, `attestor`, and `verifyAttestation(...)` result, plus the HashScan link. `NOVI_MCP_URL` and `NOVI_API_KEY` in `.env.tpl` point at prod and the prod key (task 16).
- [ ] **Step 2: LIVE against prod, the five legs, recorded in order:**
  ...
  2. `guardian-pause.mts pause --treasury <HederaDemo_1 treasury>` → `paused: true`.
  3. `demo-buyer` again → `policy denied: paused` and **no** HashScan link.
  4. `guardian-pause.mts unpause --treasury <HederaDemo_1 treasury>`, then `revoke` (guardian rotates the agent key out) → `SUCCESS`.
  ...
  **Do not call this done if** any leg differs from its expected line. Record all five with transaction ids and a screen capture each. Pending: runs after task 16 (deploy, prod key, `HederaDemo_1` linked). Legs 2 and 4 use `--treasury <HederaDemo_1 treasury>`, not `--entity`; `.env.guardian.tpl` is gitignored by design and the runbook says how to write it.
- [x] **Step 3:** Done 2026-09-12 (docs in aea3bf9 and aed0974): README demo section, `docs/runbooks/hedera-demo.md`, `docs/README.md` index. Deferred minor: `narrateReports` is duplicated from `commands/pay.ts` (export it after the hackathon). The merge step waits on step 2 and on Alex's word. The spec as written: README demo section with the five commands and expected lines; `docs/runbooks/hedera-demo.md` (the five legs, the prod URLs, the 1Password item titles, the D28 fallback, which resolvers follow the memo; audit U6); add both Hedera docs and the runbook to `docs/README.md` (audit U8). `superpowers:requesting-code-review` on the branch. **Commit** → `feat(hedera-client): demo buyer; live five-leg run recorded; demo runbook (task 15)`. **Merge step (ends phase 3):** push, `gh pr create` with title "feat(hedera): signed /verify attestation, guardian script, demo buyer (PR 3 of 3)", reviewer Martin, then D29. The continuity README and the video follow on `main`, outside this plan.
...
- 2026-09-11 Martin: design approved; deploys option 1 on integration branch `hedera`; data option 1; merge option 2. Two suggestions folded (D30, task 1 Arc leg).
- 2026-09-11 baseline on `hedera` `6d67f49`: 259 test files passed, 5 skipped; 2199 tests passed, 5 skipped; 301 s.
- 2026-09-11 `FormationE2E_1`: publicId `9f8003f5-4c70-435a-9980-9a54625691b7`, treasury `0x92ae7c6b6eB9470d7E01F8fEb352714bD80A7AAf`, agentId `886257`, standing `active`, humanVerified true.
- 2026-09-11 `Hedera Demo Guardian Key` address `0x6AB681DbFa81CA0D4acec965bDEd3A17292Ac142`; `Novi Corpus Attestation Key` address `0x269EF6d269361ceBE28D10A400E5290e7f23CdC6`; `Hedera Platform Operator` account `0.0.10412694`.
- 2026-09-11 USDC association of `0.0.10412694`: `TOKENASSOCIATE SUCCESS`, tx `0.0.10412694@1789157387.239566682`.
- 2026-09-11 testnet reset check: none scheduled through 2026-09-16.
- 2026-09-11 `HederaDemo_1`: onboarding reached the World step; prod enforces the guardian gate with no skip; waiver code requested from Martin. Values pending.
- 2026-09-11 finding on step 9: `LegalManagerFactory.createEntity` is `onlyOwner` and requires `manager == owner` (`0xb43CbdA374e3CD2a3d67827683F81462BaCF703b` on Arc testnet), so a local backend with a throwaway platform key cannot create `HederaDemo_local` against the real chain, and the sandbox profile carries no Circle configuration. Alex decided 2026-09-11: replaced by the demo-only `scripts/demo/seed-public-entity.mts` (task 6b), labeled and guarded per the global constraint.
